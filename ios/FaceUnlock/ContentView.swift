...
struct ContentView: View {
 ...
 }.navigationTitle("FaceUnlock").sheet(isPresented:$showScanner){QRScannerView{code in showScanner=false;Task{if code.contains("faceunlock-pair-v1"){await coordinator.pair(from:code)}else if let data=code.data(using:.utf8),let p=try? JSONDecoder().decode(OfflineUnlockPayload.self,from:data){do{try await coordinator.approveOfflineQR(p);coordinator.lastMessage="Face ID approved. Waiting for the PC over Bluetooth."}catch{coordinator.lastMessage=error.localizedDescription}}else{coordinator.lastMessage="Unknown QR"}}}.ignoresSafeArea()}}}
}
