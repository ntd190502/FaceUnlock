    ...
    var body: some View {
        Form {
                ...
                ForEach(apps) { app in
                    HStack {
                        VStack(alignment: .leading, spacing: 3) {
                            Text(app.name)
                            if !app.title.isEmpty { Text(app.title).font(.caption).foregroundColor(.secondary).lineLimit(2) }
                        }
                        Spacer()
                        Button("Close") { run("close_app", ["pid": "\(app.id)"]) }.buttonStyle(.borderless)
                    }
                }
            }
            ...
            Section(header: Text("Files")) {
                ...
                Text("Choose Photo Library, Camera, or Browse. For PC → iPhone, copy a file in Windows Explorer first.").font(.caption).foregroundColor(.secondary)
            }
            Section(header: Text("Status")) { Text(status) }
        }
        .navigationTitle(pc.name)
        .actionSheet(isPresented: $showFileSources) {
            ActionSheet(title: Text("Choose file source"), buttons: [
                .default(Text("Photo Library")) { showPhotoLibrary = true },
                .default(Text("Take Photo or Video")) {
                    if UIImagePickerController.isSourceTypeAvailable(.camera) { showCamera = true }
                    else { status = "Camera is unavailable" }
                },
                .default(Text("Browse")) { showPicker = true },
                .cancel()
            ])
        }
        .fileImporter(isPresented: $showPicker, allowedContentTypes: [.data], allowsMultipleSelection: false) { result in
            switch result {
            case .success(let urls): guard let url = urls.first else { status = "No file selected"; return }; sendFile(url)
            case .failure(let error): status = error.localizedDescription
            }
        }
        .sheet(isPresented: $showPhotoLibrary) { MediaPicker(sourceType: .photoLibrary) { url in showPhotoLibrary = false; if let url { sendFile(url) } } }
        .sheet(isPresented: $showCamera) { MediaPicker(sourceType: .camera) { url in showCamera = false; if let url { sendFile(url) } } }
    ...
    private func run(_ type: String, _ payload: [String: String]? = nil) {
        Task { @MainActor in
            ...
            do {
                ...
                case "close_app": if let pidText = payload?["pid"], let pid = Int(pidText) { apps.removeAll { $0.id == pid } }; status = "Application closed"
                ...
                case "screenshot":
                    guard result?.available != false else { status = result?.error ?? "Screenshot is unavailable"; return }
                    guard let b64 = result?.base64, let data = Data(base64Encoded: b64), let decoded = UIImage(data: data) else { status = "Screenshot data is invalid"; return }
                    image = decoded; status = "Screenshot updated"
                case "clipboard_file_download":
                    guard result?.available == true else { status = "No copied file found on PC. Copy a file in Windows Explorer first."; return }
                    guard let b64 = result?.base64, let data = Data(base64Encoded: b64) else { status = "Downloaded file data is invalid"; return }
                    let safeName = URL(fileURLWithPath: result?.name ?? "FaceUnlock-file").lastPathComponent
                    let directory = FileManager.default.temporaryDirectory.appendingPathComponent("FaceUnlockDownloads", isDirectory: true)
                    try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
                    let url = directory.appendingPathComponent(safeName); try? FileManager.default.removeItem(at: url); try data.write(to: url, options: .atomic)
                    shareURL = url; showShareSheet = true; status = "File ready to save: \(safeName)"
                case "lock": status = result?.locked == true ? "PC locked" : "Lock command completed"
    ...
    private func sendFile(_ url: URL) {
        Task { @MainActor in
            ...
            do {
                ...
                guard values.isRegularFile == true else { status = "Please select a file, not a folder"; return }
                if let fileSize = values.fileSize, fileSize > 8 * 1024 * 1024 { status = "File exceeds 8 MB remote relay limit"; return }
                ...
                if response.status == "ERROR" { status = response.result?.error ?? "File transfer failed"; return }
...
struct MediaPicker: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UIImagePickerController {
        let picker = UIImagePickerController(); picker.sourceType = sourceType; picker.mediaTypes = [UTType.image.identifier, UTType.movie.identifier]; picker.videoQuality = .typeMedium; picker.delegate = context.coordinator; return picker
    }
    ...
    final class Coordinator: NSObject, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
        ...
        func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
            if let mediaURL = info[.mediaURL] as? URL { completion(mediaURL); return }
            guard let image = info[.originalImage] as? UIImage, let data = image.jpegData(compressionQuality: 0.9) else { completion(nil); return }
            let url = FileManager.default.temporaryDirectory.appendingPathComponent("FaceUnlock-\(UUID().uuidString).jpg")
