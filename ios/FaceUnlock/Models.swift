...
struct RemoteResult: Codable { let cpu_percent:Double?; let ram_percent:Double?; let temperature_c:Double?; let locked:Bool?; let accepted:Bool?; let closed:Bool?; let text:String?; let available:Bool?; let mime:String?; let base64:String?; let name:String?; let size:Int?; let error:String?; let apps:[RemoteApp]? }
struct RemoteApp: Codable,Identifiable { let id:Int; let name:String; let title:String }
struct HostedFile: Codable,Identifiable { let id:String; let name:String; let size:Int64; let mime:String?; let created_at:String? }
struct HostedFileList: Codable { let ok:Bool; let files:[HostedFile] }
