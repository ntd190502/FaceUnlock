...
final class APIClient {
 ...
 private func request<T:Decodable>(_ path:String,method:String="GET",token:String?=nil,body:Encodable?=nil,baseURL:String?=nil) async throws->T{
  ...
  catch let DecodingError.keyNotFound(key,_){throw NSError(domain:"FaceUnlock.API",code:-2,userInfo:[NSLocalizedDescriptionKey:"Hosting response is missing required field: \(key.stringValue)"])}
  catch let DecodingError.valueNotFound(_,context){throw NSError(domain:"FaceUnlock.API",code:-2,userInfo:[NSLocalizedDescriptionKey:"Hosting response has null required field: \(context.codingPath.last?.stringValue ?? "unknown")"])}
  catch let DecodingError.typeMismatch(_,context){throw NSError(domain:"FaceUnlock.API",code:-2,userInfo:[NSLocalizedDescriptionKey:"Hosting response has invalid field type: \(context.codingPath.last?.stringValue ?? "unknown")"])}
  catch{throw NSError(domain:"FaceUnlock.API",code:-2,userInfo:[NSLocalizedDescriptionKey:"Hosting response could not be decoded safely"])}
 }
 @MainActor func completePair(_ payload:PairingPayload) async throws->PairCompleteResponse{let req=PairCompleteRequest(pair_id:payload.pair_id,pair_code:payload.pair_code,iphone_name:UIDevice.current.name,iphone_public_key_pem:try DeviceKey.shared.publicKeyPEM());return try await request("/v1/pair/complete",method:"POST",body:req,baseURL:payload.server)}
