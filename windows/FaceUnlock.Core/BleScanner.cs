...
public sealed class BleScanner
{
    ...
    private async Task<OfflineBleResponse?> DiscoverAndApproveExclusiveAsync(OfflineUnlockPayload payload, string? expectedDeviceId, TimeSpan timeout, CancellationToken ct)
    {
        ...
        // Radio.State can report Enabled before the Windows BLE/GATT stack is ready.
        // Only delay when FaceUnlock actually had to turn Bluetooth on; do not slow
        // down the normal path when the user already had Bluetooth enabled.
        ...
        try
        {
            while (DateTime.UtcNow < deadline && !ct.IsCancellationRequested)
            {
                ...
                while (addressQueue.TryDequeue(out var address))
                {
                    ...
                    if (result != null)
                    {
                        BleDiag(payload, "RESPONSE_COMPLETE");
                        return result;
                    }
                }
            }
    ...
    private static async Task<OfflineBleResponse?> TryConnectAndApproveAsync(ulong address, OfflineUnlockPayload payload, string? expectedDeviceId, TimeSpan timeout, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (services.Status != GattCommunicationStatus.Success || services.Services.Count == 0 || DateTime.UtcNow >= operationDeadline)
            {
                BleDiag(payload, $"SERVICE_FOUND fail status={services.Status} count={services.Services.Count}");
                return null;
            }
            ...
            if (!string.IsNullOrWhiteSpace(expectedDeviceId))
            {
                ...
                if (devChars.Status != GattCommunicationStatus.Success || devChars.Characteristics.Count == 0 || DateTime.UtcNow >= operationDeadline)
                {
                    BleDiag(payload, $"DEVICE_ID fail stage=characteristic status={devChars.Status}");
                    return null;
                }
                ...
                {
                    BleDiag(payload, $"DEVICE_ID fail stage=read status={read.Status}");
                    return null;
                }
                ...
                if (!string.Equals(Encoding.UTF8.GetString(idBytes).Trim(), expectedDeviceId, StringComparison.Ordinal))
                {
                    BleDiag(payload, "DEVICE_ID mismatch");
                    return null;
                }
            ...
            if (reqResult.Status != GattCommunicationStatus.Success || resResult.Status != GattCommunicationStatus.Success ||
                reqResult.Characteristics.Count == 0 || resResult.Characteristics.Count == 0 || DateTime.UtcNow >= operationDeadline)
            {
                BleDiag(payload, $"CHARACTERISTICS fail request={reqResult.Status} response={resResult.Status}");
                return null;
            }
            ...
            TypedEventHandler<GattCharacteristic, GattValueChangedEventArgs> handler = (_, e) =>
            {
                try
                {
                    ...
                    {
                        BleDiag(payload, $"RESPONSE_RECEIVED bytes={complete.Length}");
                        responseTcs.TrySetResult(complete);
                    }
                    else if (status == BleAssemblyStatus.Invalid)
                    {
                        BleDiag(payload, $"RESPONSE_INVALID error={Sanitize(error)}");
                        responseTcs.TrySetException(new InvalidDataException(error ?? "Invalid BLE response frame."));
                    }
                }
            ...
            try
            {
                ...
                if (cccdStatus != GattCommunicationStatus.Success || DateTime.UtcNow >= operationDeadline)
                {
                    BleDiag(payload, $"NOTIFY_SUBSCRIBED fail status={cccdStatus}");
                    return null;
                }
                ...
                foreach (var frame in frames)
                {
                    ...
                    {
                        BleDiag(payload, $"REQUEST_WRITE fail frame={frameNo} status={write.Status}");
                        return null;
                    }
                }
                ...
                if (done != responseTcs.Task)
                {
                    BleDiag(payload, "RESPONSE_TIMEOUT");
                    return null;
                }
        ...
        {
            BleDiag(payload, $"GATT_EXCEPTION type={ex.GetType().Name} message={Sanitize(ex.Message)}");
            return null;
        }
    }
    ...
    private static void BleDiag(OfflineUnlockPayload payload, string message)
    {
        try
        {
            ...
            lock (DiagnosticSync)
            {
                ...
                if (File.Exists(path) && new FileInfo(path).Length >= MaxDiagnosticBytes)
                    File.WriteAllText(path, string.Empty);
                File.AppendAllText(path, $"[{DateTime.UtcNow:yyyy-MM-dd HH:mm:ss.fffZ}] request={Short(payload.LogicalRequestId)} session={Short(payload.Session)} {message}{Environment.NewLine}", Encoding.UTF8);
            }
        }
        catch { }
    }
