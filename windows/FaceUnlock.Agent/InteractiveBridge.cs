...
public sealed class InteractiveBridge:IDisposable{
 ...
 void Tick(){try{var input=F("clipboard-in.txt");if(File.Exists(input)){var t=File.GetLastWriteTimeUtc(input);if(t>_lastClipboardIn){System.Windows.Clipboard.SetText(File.ReadAllText(input));_lastClipboardIn=t;}}var incoming=F("incoming-file.path");if(File.Exists(incoming)){var t=File.GetLastWriteTimeUtc(incoming);if(t>_lastIncoming){var p=File.ReadAllText(incoming).Trim();if(File.Exists(p)){var c=new StringCollection();c.Add(p);System.Windows.Clipboard.SetFileDropList(c);}_lastIncoming=t;}}if(System.Windows.Clipboard.ContainsText())File.WriteAllText(F("clipboard-out.txt"),System.Windows.Clipboard.GetText());if(System.Windows.Clipboard.ContainsFileDropList()){var files=System.Windows.Clipboard.GetFileDropList();if(files.Count>0)File.WriteAllText(F("clipboard-file.path"),files[0]!);}var req=F("screenshot.request");if(File.Exists(req)){Capture(F("screenshot.jpg"));File.Delete(req);}}catch{}}
