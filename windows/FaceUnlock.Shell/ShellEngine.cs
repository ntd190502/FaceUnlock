...
public sealed class ShellEngine
{
    ...
    public async Task InitializeAndAutoStartAsync(CancellationToken ct = default, int maxRetries = 15)
    {
        ...
        // Keep the shell alive through service start races, network changes and
        // rejected/expired attempts. One attempt is active at a time.
        ...
        while (!ct.IsCancellationRequested && !_explorerStarted)
        {
            ...
            for (var i = 0; i < maxRetries && !ct.IsCancellationRequested; i++)
            {
                ...
                if (ping?.status == LocalAuthStatus.Ok)
                {
                    serviceOk = true;
                    break;
                }
            ...
            Log("Service connected. Starting Face ID unlock request.");
            var approved = await TryStartFaceIdAttemptAsync(ct);
            if (approved || _explorerStarted || ct.IsCancellationRequested) return;

            // NOT_PAIRED is usually not transient, so retry more slowly. Other
            // states recover quickly after connectivity/Face ID changes.
    ...
    public async Task<bool> TryStartFaceIdAttemptAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            var req = new LocalAuthRequest(1, "request_unlock", reqId,
            ...
            {
                SetState(ShellState.ERROR, ack?.message ?? "Service communication failed. Retrying automatically...");
                return false;
            }
            if (ack.status == LocalAuthStatus.NotPaired)
            {
                SetState(ShellState.NOT_PAIRED, "FaceUnlock is not paired with iPhone. Waiting for setup...");
                return false;
            }
            ...
            while (!attemptToken.IsCancellationRequested)
            {
                ...
                var statusResp = await SendIpcAsync(new LocalAuthRequest(1, "grant_status", reqId,
                    user_sid: sid, session_id: session, client_type: shellClientType, process_id: _processId), 2000);
                if (statusResp == null) continue;

                switch (statusResp.status)
                {
                    case LocalAuthStatus.Approved:
                        return await HandleApprovalFlowAsync(reqId, sid, session, attemptToken);
                    case LocalAuthStatus.Rejected:
                        SetState(ShellState.REJECTED, "Request declined. Retrying automatically...");
                        return false;
                    case LocalAuthStatus.Timeout:
                    case LocalAuthStatus.Expired:
                        SetState(ShellState.TIMEOUT, "Request timed out. Retrying automatically...");
                        return false;
                    case LocalAuthStatus.NotPaired:
                        SetState(ShellState.NOT_PAIRED, "FaceUnlock is not paired with iPhone. Waiting for setup...");
                        return false;
                    case LocalAuthStatus.WaitingConnectivity:
                    ...
                    case LocalAuthStatus.Error:
                        SetState(ShellState.ERROR, statusResp.message ?? "Authorization error. Retrying automatically...");
                        return false;
                }
            }
            return false;
        ...
        {
            await SendIpcAsync(new LocalAuthRequest(1, "cancel_request", reqId,
                session_id: GetCurrentWindowsSessionId(), client_type: shellClientType, process_id: _processId), 1500);
            return false;
        }
        catch (Exception ex)
        {
            Log($"Exception during Face ID attempt: {ex.Message}");
            SetState(ShellState.ERROR, "Unexpected error. Retrying automatically...");
            return false;
        }
        finally
        {
            lock (_stateLock) _isAttemptInProgress = false;
        }
    }
    ...
    private async Task<bool> HandleApprovalFlowAsync(string reqId, string? sid, int session, CancellationToken ct)
    {
        ...
        var reserveResp = await SendIpcAsync(new LocalAuthRequest(1, "reserve_grant", reqId,
            user_sid: sid, session_id: session, client_type: clientType, process_id: _processId), 3000);
        if (reserveResp?.status != LocalAuthStatus.Reserved)
        {
            SetState(ShellState.ERROR, "Authorization reservation failed. Retrying automatically...");
            return false;
        }
        if (!string.IsNullOrWhiteSpace(reserveResp.user_sid) && !string.IsNullOrWhiteSpace(sid)
            && !string.Equals(reserveResp.user_sid, sid, StringComparison.OrdinalIgnoreCase))
        {
            SetState(ShellState.ERROR, "Authorization binding mismatch.");
            return false;
        }

        var consumeResp = await SendIpcAsync(new LocalAuthRequest(1, "consume_grant", reqId,
            user_sid: sid, session_id: session, client_type: clientType, process_id: _processId), 3000);
        if (consumeResp?.status != LocalAuthStatus.Consumed
            || !string.Equals(consumeResp.user_sid, sid, StringComparison.OrdinalIgnoreCase)
            || consumeResp.session_id != session)
        {
            SetState(ShellState.ERROR, "Authorization consumption failed. Retrying automatically...");
            return false;
        }
    ...
    internal bool CompleteApprovedGrantAndLaunchExplorer()
    {
        lock (_stateLock)
        {
            if (_inputGuardFailed) return false;
    ...
    public bool RetryExplorerSafe()
    {
        lock (_stateLock)
        {
            if (!_desktopReleaseAuthorized || _inputGuardFailed) return false;
        }
        return LaunchExplorerSafe();
    }
    ...
    private bool LaunchExplorerSafe()
    {
        lock (_stateLock)
        {
            if (_explorerStarted) return true;
        ...
        if (!_launcher.FileExists(explorerPath))
        {
            ...
            SetState(ShellState.DESKTOP_FAILED, "Windows Desktop executable was not found. Retrying automatically...");
            _ = RetryDesktopWithBackoffAsync();
            return false;
        }
        ...
        lock (_stateLock) _explorerStarted = false;
        SetState(ShellState.DESKTOP_FAILED, "Desktop failed to start. Retrying automatically...");
        _ = RetryDesktopWithBackoffAsync();
        return false;
    }
    ...
    private bool EnsureInputGuard()
    {
        ...
        {
            if (_inputGuardFailed) return false;
            if (_inputGuardAttempted) return _inputGuard.IsActive;
