...
public sealed class RemoteControlWorker : BackgroundService
{
    ...
    sealed class LockBridgeResponse { public string request_id { get; set; } = ""; public bool ok { get; set; } public string? error { get; set; } }
    ...
    static object ReadStatus()
    {
        ...
        try
        {
            ...
            var values = searcher.Get().Cast<ManagementObject>().Select(x => Convert.ToDouble(x["LoadPercentage"])).ToArray();
    ...
    static double? CpuTemperature()
    {
        try
        {
            ...
            var values = searcher.Get().Cast<ManagementObject>().Select(x => (Convert.ToDouble(x["CurrentTemperature"]) / 10) - 273.15).Where(x => x > 0 && x < 150).ToArray();
    ...
    static object Lock()
    {
        ...
        var resultFile = Bridge("lock.result.json");
        TryDelete(resultFile);
        File.WriteAllText(Bridge("lock.request"), requestId);
        for (var i = 0; i < 40; i++)
        {
            Thread.Sleep(100);
            ...
            try
            {
                var response = JsonSerializer.Deserialize<LockBridgeResponse>(File.ReadAllText(resultFile));
                if (response?.request_id != requestId) continue;
                if (!response.ok) throw new InvalidOperationException(response.error ?? "Could not lock workstation");
                return new { locked = true, mode = "LockWorkStation" };
            }
            catch (IOException) { }
            catch (JsonException) { }
        }
        throw new InvalidOperationException("FaceUnlock Agent interactive bridge did not respond to lock request");
    }
    ...
    static object Apps()
    {
        var requestId = Guid.NewGuid().ToString("N"); var resultFile = Bridge("apps.result.json"); TryDelete(resultFile); File.WriteAllText(Bridge("apps.request"), requestId);
        for (var i = 0; i < 40; i++)
        {
            Thread.Sleep(100); if (!File.Exists(resultFile)) continue;
            try { var response = JsonSerializer.Deserialize<AppsBridgeResponse>(File.ReadAllText(resultFile)); if (response?.request_id == requestId) return new { apps = response.apps }; }
    ...
    static object CloseApp(Dictionary<string, object>? payload)
    {
        ...
        var requestId = Guid.NewGuid().ToString("N"); var resultFile = Bridge("close-app.result.json"); TryDelete(resultFile);
        ...
        for (var i = 0; i < 40; i++)
        {
            ...
            try
            {
                var response = JsonSerializer.Deserialize<CloseBridgeResponse>(File.ReadAllText(resultFile)); if (response?.request_id != requestId) continue;
                if (!response.ok) throw new InvalidOperationException(response.error ?? "Could not close application"); return new { closed = true, pid };
            }
            catch (IOException) { } catch (JsonException) { }
        }
    ...
    static object ClipboardSet(Dictionary<string, object>? payload)
    {
        ...
        File.WriteAllText(Bridge("clipboard-in.txt"), text); return new { queued = true };
    }

    static object ClipboardGet()
    {
        var file = Bridge("clipboard-out.txt"); return new { text = File.Exists(file) ? File.ReadAllText(file) : "", available = File.Exists(file) };
    }

    static object FileUpload(Dictionary<string, object>? payload)
    {
        if (payload == null || !payload.TryGetValue("name", out var nameValue) || !payload.TryGetValue("base64", out var base64Value)) throw new InvalidOperationException("name/base64 required");
        var name = Path.GetFileName(nameValue?.ToString() ?? "upload.bin"); var data = Convert.FromBase64String(base64Value?.ToString() ?? "");
        if (data.Length > 8 * 1024 * 1024) throw new InvalidOperationException("File exceeds 8 MB remote relay limit");
        var directory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "FaceUnlock", "Incoming"); Directory.CreateDirectory(directory);
        var path = Path.Combine(directory, name); File.WriteAllBytes(path, data); File.WriteAllText(Bridge("incoming-file.path"), path);
        return new { saved = true, name, size = data.Length };
    }

    static object ClipboardFile()
    {
        var marker = Bridge("clipboard-file.path"); if (!File.Exists(marker)) return new { available = false };
        var path = File.ReadAllText(marker).Trim(); if (!File.Exists(path)) return new { available = false };
        var data = File.ReadAllBytes(path); if (data.Length > 8 * 1024 * 1024) throw new InvalidOperationException("File exceeds 8 MB remote relay limit");
        return new { available = true, name = Path.GetFileName(path), base64 = Convert.ToBase64String(data), size = data.Length };
    }

    static object Screenshot()
    {
        var request = Bridge("screenshot.request"); var file = Bridge("screenshot.jpg"); TryDelete(file); File.WriteAllText(request, DateTime.UtcNow.Ticks.ToString());
        for (var i = 0; i < 30 && !File.Exists(file); i++) Thread.Sleep(100);
        if (!File.Exists(file)) return new { available = false, error = "FaceUnlock Agent interactive bridge is not running" };
        var data = File.ReadAllBytes(file); return new { available = true, mime = "image/jpeg", base64 = Convert.ToBase64String(data) };
    }
    ...
    async Task CheckAlerts(LocalConfig config, CancellationToken ct)
    {
        ...
        {
            _lastRamAlert = now; await Telegram(config, $"⚠️ FaceUnlock RAM alert\nPC: {config.PcName}\nRAM: {ram:F1}% (limit {config.RamAlertPercent:F0}%)", ct);
        }
        ...
        {
            _lastTempAlert = now; await Telegram(config, $"🔥 FaceUnlock temperature alert\nPC: {config.PcName}\nCPU: {temp:F1}°C (limit {config.TemperatureAlertCelsius:F0}°C)", ct);
        }
    }
    ...
    static async Task Telegram(LocalConfig config, string text, CancellationToken ct)
    {
        ...
        using var client = new HttpClient(); await client.PostAsJsonAsync($"https://api.telegram.org/bot{config.TelegramBotToken}/sendMessage", new { chat_id = config.TelegramChatId, text }, ct);
    }
}
